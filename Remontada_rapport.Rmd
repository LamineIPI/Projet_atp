...
En utilisant l’échantillonnage aléatoire on a eu des résultats insatisfaisants en termes de prédiction, en essayant d’ajuster les modèles de la meilleure façon possible (à travers le choix des hyperparamètres qui permettent de minimiser l’erreur de prévision). Le modèle de bagging va nous permettre de détecter les variables qui expliquent les retournements de situations ou non. 
...
Après la filtration de nos données, nous avons choisi de répartir les matchs non remontada en 2 clusters (Annexe 7 et 8).Toutes les méthodes testés (Elbow, silhouette et dix autres) ont suggéré une décomposition en deux clusters, regroupant ainsi les observations les plus semblables entre elles.
...
Cette fois-ci le bagging a donné le meilleur taux de mauvaise classification, soit `r tx_err_bagging2`.Au niveau de notre échantillon globale le modèle du bagging permet de classer correctement dans 73% des cas les matchs présentant un retournement de situation. Cependant, le modèle ne classifie pas bien les matchs sans remontada dans 49% des cas.
...
Don on peut retenir la classification par bagging (bootstrap agregation) afin de prendre les quatre meilleures variables qui expliquent un retournement de situation : 
