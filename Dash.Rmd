...
```{r}
atp_Nadal%>%filter(loser_name==paste(J_Nom,J_Prenom,sep=" "))%>%select(winner_rank)%>%mutate(classe=cut(winner_rank,c(0,5,max(winner_rank))))%>%group_by(classe)%>%summarise(NB_classe=n())%>%mutate(freq=round((NB_classe/sum(NB_classe))*100))->Winner_Rank
