...
Max_ace  <- max(W_Nadal$w_ace,na.rm= TRUE)
valueBox(Max_ace, icon = "fa-baseball-ball", color = "Blue")
```

### Le maximum des doubles fautes
```{r echo=FALSE}
atp_Nadal%>%mutate(Nadal_win=ifelse(winner_name==paste(J_Prenom,J_Nom,sep=" "),1,0))->D
...
#calculer le taux des gains pour les top10 
atp_matches_2013 %>%
  filter(winner_id != id_Nadal & winner_rank < 11) %>%
  group_by(Surface) %>%
  summarize(nombre_victoire = n()) %>%
  mutate(nb_match =nbr_game_top10$nombre_victoire, taux_top10 = round(nombre_victoire/nb_match,2)) %>%
  as.data.frame() -> nbr_game_top10_win 

taux_top10 <- nbr_game_top10_win[,4]

#calculer le taux des gains pour Rafael Nadal

atp_Nadal %>% 
  filter(winner_id == id_Nadal) %>%
  group_by(Surface) %>%
  summarize(Nombre_victoire =n()) -> Nbr_victoire_Nadal

n = nrow(Nbr_victoire_Nadal)
if (n < 3){
  for(i in 1:3-n){
  Nbr_victoire_Nadal <- rbind(Nbr_victoire_Nadal,c("Herbe",0))
  break
  }
}

Nbr_victoire_Nadal$Nombre_victoire = as.numeric(Nbr_victoire_Nadal$Nombre_victoire)

atp_matches_2013 %>% 
  filter(winner_id == id_Nadal | loser_id == id_Nadal)%>%
  group_by(Surface) %>% 
  summarize(Nbr_match = n())->Nombre_match_Nadal

Nombre_match_Nadal %>%
  inner_join(Nbr_victoire_Nadal,by="Surface") -> Res_Nadal

Res_Nadal %>%
  mutate(taux_nadal = round(Nombre_victoire/Nbr_match,2)) -> Res_NadalVsTop10

taux_Nadal <- Res_NadalVsTop10$taux_nadal
...
min <- rep(0,each=3)
max <- rep(1,each=3)
tible <- t(cbind(max,min,taux_Nadal,taux_top10))
tible <- as.data.frame(tible)
colnames(tible) <- c("Victoire sur \n terre dure","Victoire sur terre \n gazon","Victoire \n sur battue")
...
legend(-2,1.25, legend=c("Top 10",J_Nom),
...
col<-c("id","J_Prenom1","J_Nom1","hand","Bth","Nat")
...
players%>%
  filter(J_Prenom1==J_Prenom & J_Nom1==J_Nom)%>%
  select(id)%>%
  as.numeric()->id_Nadal
...
```{r echo=FALSE}
valueBox(nat,icon = "fa-table-tennis",color = "green")
```
...
### Classement  actuel {data-width=10, data-height=10}
```{r echo=FALSE}
# classement mondial actuel
atp_classement%>%
  arrange(desc(ranking_date)) %>%
  filter(player==id_Nadal)%>%
  select(rank)%>%
  slice(1L)%>%
